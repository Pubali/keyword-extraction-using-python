...
def keyword_extraction_tfidf(texts, method='phrase', min_df=5, max_df=0.8, num_key=20):
    """
    ...
    vocabulary = [generate_keyword(unidecode(text), method=method) for text in texts]
...
if __name__ == '__main__':
    import pandas as pd
    # texts = list(pd.read_csv('data/Resumes.txt')['resume'])
    fp = io.open('data/Resumes.txt','r',encoding='latin-1').read()
    text = fp.split(' ')
    key_phrases = keyword_extraction_tfidf(text)
